...
public class AES128 extends CircuitGenerator {
    ...
    public void setPlaintext(UnsignedInteger[] pt) {
        plaintext = pt;
    }
    public void setKey(UnsignedInteger[] k) {
        key = k;
    }
    public UnsignedInteger[] getCiphertext() {
        return ciphertext;
    }
    ...
    private UnsignedInteger gal_mul_const(UnsignedInteger x, int c) {
        ...
        for (int counter = 0; counter < 8; counter += 1) {
            ...
            {
                Bit bit_l0c0tb = hi.copy();
                boolean c_l0c0tb = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_l0c0tb);
                if (c_l0c0tb) {
                    if (bit_l0c0tb.getConstantValue()) {
                        x.assign(tmp, 8);
                    } else {
                    ...
                    ConditionalScopeTracker.push(bit_l0c0tb);
                    x.assign(tmp, 8);
